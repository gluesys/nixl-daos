...
cd /nixl
if ! meson setup build-daos --reconfigure -Ddaos_path=/usr -Ddisable_gds_backend=true -Ddisable_mooncake_backend=true -Ddisable_infinia_backend=true -Dbuildtype=debugoptimized -Dbuild_tests=true > build-daos-setup.log 2>&1; then
